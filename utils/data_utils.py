...
def load_data_partseg(partition, dataset_path):
...
def prepare_test_data_semseg(DATA_DIR):
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    # DATA_DIR = os.path.join(BASE_DIR, 'data')
    if not os.path.exists(os.path.join(DATA_DIR, 'stanford_indoor3d')):
        os.system(f'python {DATA_DIR}/prepare_data/collect_indoor3d_data.py')
    if not os.path.exists(os.path.join(DATA_DIR, 'indoor3d_sem_seg_hdf5_data')):
        os.system(f'python {DATA_DIR}/prepare_data/gen_indoor3d_h5.py')


def load_data_semseg_spg(partition, test_area):
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    DATA_DIR = os.path.join(BASE_DIR, 'data')
    # download_S3DIS()
    ...
    if partition == 'train':
        ...
        data_dir = os.path.join(DATA_DIR, f'indoor3d_sem_seg_hdf5_data_{partition}')
    with open(os.path.join(data_dir, "all_files.txt")) as f:
        all_files = [line.rstrip() for line in f]
    with open(os.path.join(data_dir, "room_filelist.txt")) as f:
        room_filelist = [line.rstrip() for line in f]
    data_batchlist, label_batchlist, spg_batchlist = [], [], []
    for f in all_files:
        file = h5py.File(os.path.join(DATA_DIR, f), 'r+')
        data = file["data"][:]
        label = file["label"][:]
        spg_info = file["spg_info"][:]
        data_batchlist.append(data)
        label_batchlist.append(label)
        spg_batchlist.append(spg_info)
    data_batches = np.concatenate(data_batchlist, 0)
    seg_batches = np.concatenate(label_batchlist, 0)
    spg_batches = np.concatenate(spg_batchlist, 0)
    ...
    if partition == 'train':
        ...
        all_spg = spg_batches[train_idxs, ...]
    else:
        all_data = data_batches[test_idxs, ...]
        all_seg = seg_batches[test_idxs, ...]
        all_spg = spg_batches[test_idxs, ...]
    return all_data, all_seg, all_spg


def load_data_semseg(dataset_path, partition, test_area):
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    DATA_DIR = dataset_path
    # download_S3DIS()
    prepare_test_data_semseg(DATA_DIR)
    data_dir = os.path.join(DATA_DIR, 'indoor3d_sem_seg_hdf5_data')
    # if partition == 'train':
    #     data_dir = os.path.join(DATA_DIR, 'indoor3d_sem_seg_hdf5_data')
    # else:
...
def load_color_partseg():
    colors = []
    labels = []
    f = open("utils/partseg_colors.txt")
    ...
    zipped = zip(np.arange(partseg_colors.shape[0]), partseg_colors)
    label_color_map = dict(list(zipped))
...
def load_color_semseg(dataset_path):
    colors = []
    labels = []
    f = open(f"{dataset_path}/indoor3d_sem_seg_hdf5_data/semseg_colors.txt")
    for line in json.load(f):
        colors.append(line['color'])
        labels.append(line['label'])
...
class ShapeNetPart(Dataset):
    def __init__(self, num_points, dataset_path=None, partition='train', class_choice=None):
        self.data, self.label, self.seg = load_data_partseg(partition, dataset_path)
...
class S3DIS(Dataset):
    def __init__(self, dataset_path, num_points=4096, partition='train', test_area='1'):
        self.data, self.seg = load_data_semseg(dataset_path, partition, test_area)
        self.num_points = num_points
        self.partition = partition
        self.semseg_colors = load_color_semseg(dataset_path)
