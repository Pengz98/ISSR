...
def room2blocks(data, num_blocks=2):
    ...
    """
    point, label, feat = data.point.numpy(), data.label.numpy(), data.feat.numpy()
    limit = np.amax(point, 0)[0:3]
    _limit = np.amin(point, 0)[0:3]

    axis = limit[0:2].argmax()
    block_size = (limit[0:2]-_limit[0:2]).max() / num_blocks
    stride = block_size

    # Get the corner location for our sampling blocks

    beg_list = []
    num_block = int(np.ceil((limit[axis] - _limit[axis] - block_size) / stride)) + 1
    for i in range(num_block):
        beg_list.append(i * stride)
    ...
    for idx in range(len(beg_list)):
        beg = beg_list[idx]
        cond = (point[:, axis] <= _limit[axis].min() + beg + block_size) & (point[:, axis] >= _limit[axis].min() + beg)
        block_data = point[cond, :]
        block_label = label[cond]
        block_feat = feat[cond, :]

        block_data_list.append(block_data)
        block_label_list.append(block_label)
        block_feat_list.append(block_feat)

    return block_data_list, block_label_list, block_feat_list
